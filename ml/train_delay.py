#!/usr/bin/env python3
"""KROK 2: ten sam szkielet co train.py, ale etykieta to prawdziwe, zaobserwowane
opóźnienie na mecie kursu (nie coś znane z góry). Baseline: "opóźnienie się nie zmienia".

Uruchomienie: python3 ml/prepare_delay_pairs.py && python3 ml/train_delay.py
"""
import csv
import random
from pathlib import Path

DATASET_PATH = Path(__file__).parent / "delay_pairs.csv"
TEST_FRACTION = 0.2
EPOCHS = 300
LEARNING_RATE = 0.05
TOP_CATEGORIES = 8
SEED = 42


def load_rows():
    with DATASET_PATH.open(encoding="utf-8") as f:
        return list(csv.DictReader(f))


def train_test_split(rows):
    rng = random.Random(SEED)
    shuffled = rows[:]
    rng.shuffle(shuffled)
    cut = int(len(shuffled) * (1 - TEST_FRACTION))
    return shuffled[:cut], shuffled[cut:]


def persistence_baseline(row):
    """'Opóźnienie się nie zmienia' — najprostszy sensowny baseline dla tego zadania."""
    return float(row["current_delay_sec"])


def build_feature_encoder(train_rows):
    counts = {}
    for r in train_rows:
        counts[r["route"]] = counts.get(r["route"], 0) + 1
    top = sorted(counts, key=counts.get, reverse=True)[:TOP_CATEGORIES]
    categories = top + ["INNA"]

    def raw_features(row):
        cat = row["route"] if row["route"] in top else "INNA"
        one_hot = [1.0 if cat == c else 0.0 for c in categories]
        return [
            float(row["current_delay_sec"]),
            float(row["remaining_scheduled_sec"]),
            float(row["stops_remaining"]),
            float(row["hour"]),
        ] + one_hot

    feature_names = ["current_delay_sec", "remaining_scheduled_sec", "stops_remaining", "hour"] + [
        f"kategoria={c}" for c in categories
    ]
    return raw_features, feature_names


def standardize(matrix):
    n_cols = len(matrix[0])
    means = [sum(row[j] for row in matrix) / len(matrix) for j in range(n_cols)]
    stds = []
    for j in range(n_cols):
        variance = sum((row[j] - means[j]) ** 2 for row in matrix) / len(matrix)
        stds.append(variance ** 0.5 or 1.0)
    scaled = [[(row[j] - means[j]) / stds[j] for j in range(n_cols)] for row in matrix]
    return scaled, means, stds


def train_linear_regression(X, y, feature_names):
    n, d = len(X), len(X[0])
    weights = [0.0] * d
    bias = 0.0
    for epoch in range(1, EPOCHS + 1):
        preds = [sum(w * x for w, x in zip(weights, row)) + bias for row in X]
        errors = [p - target for p, target in zip(preds, y)]
        grad_w = [sum(e * X[i][j] for i, e in enumerate(errors)) / n for j in range(d)]
        grad_b = sum(errors) / n
        weights = [w - LEARNING_RATE * g for w, g in zip(weights, grad_w)]
        bias -= LEARNING_RATE * grad_b
        if epoch == 1 or epoch % 50 == 0 or epoch == EPOCHS:
            mse = sum(e * e for e in errors) / n
            print(f"  epoka {epoch:>4}: MSE = {mse:,.0f}  (RMSE ~ {mse ** 0.5 / 60:.2f} min)")

    print("\nNauczone wagi (po standaryzacji cech):")
    for name, w in sorted(zip(feature_names, weights), key=lambda p: -abs(p[1])):
        kierunek = "zwiększa przewidywane opóźnienie" if w > 0 else "zmniejsza przewidywane opóźnienie"
        print(f"  {name:<24} {w:+7.1f}  ({kierunek})")
    return weights, bias


def predict_linear(row_scaled, weights, bias):
    return sum(w * x for w, x in zip(weights, row_scaled)) + bias


def mae_minutes(predictions, actual):
    return sum(abs(p - a) for p, a in zip(predictions, actual)) / len(actual) / 60


def main():
    rows = load_rows()
    if not rows:
        print(f"{DATASET_PATH} jest puste — najpierw uruchom ml/prepare_delay_pairs.py.")
        return
    train_rows, test_rows = train_test_split(rows)
    print(f"Przykłady (punkt na trasie -> opóźnienie na mecie): {len(rows)} (trening: {len(train_rows)}, test: {len(test_rows)})\n")

    print("=== Baseline: 'opóźnienie się nie zmienia' ===")
    baseline_preds = [persistence_baseline(r) for r in test_rows]
    actual = [float(r["final_delay_sec"]) for r in test_rows]
    baseline_mae = mae_minutes(baseline_preds, actual)
    print(f"Baseline MAE (test): {baseline_mae:.2f} min\n")

    print("=== Regresja liniowa (spadek gradientu, od zera) ===")
    encode, feature_names = build_feature_encoder(train_rows)
    X_train_raw = [encode(r) for r in train_rows]
    y_train = [float(r["final_delay_sec"]) for r in train_rows]
    X_train, means, stds = standardize(X_train_raw)
    weights, bias = train_linear_regression(X_train, y_train, feature_names)

    X_test_raw = [encode(r) for r in test_rows]
    X_test = [[(x - m) / s for x, m, s in zip(row, means, stds)] for row in X_test_raw]
    model_preds = [predict_linear(row, weights, bias) for row in X_test]
    model_mae = mae_minutes(model_preds, actual)

    print(f"\nModel MAE (test): {model_mae:.2f} min")
    print(f"Baseline MAE (test): {baseline_mae:.2f} min")
    improvement = (1 - model_mae / baseline_mae) * 100
    if improvement > 0:
        print(f"-> model jest o {improvement:.1f}% dokładniejszy niż baseline")
    else:
        print(f"-> model jest GORSZY od baseline'u o {-improvement:.1f}% — 'opóźnienie się nie zmienia' "
              f"okazało się trudne do pobicia (to też ważny, uczciwy wynik)")

    print("\n=== 5 przykładowych przewidywań (test) ===")
    for row, pred, act in list(zip(test_rows, model_preds, actual))[:5]:
        print(f"  {row['route']:<5} spóźniony teraz {float(row['current_delay_sec'])/60:+.1f} min, "
              f"jeszcze {row['stops_remaining']} przystanków -> "
              f"na mecie rzeczywiście: {act/60:+.1f} min, model: {pred/60:+.1f} min")


if __name__ == "__main__":
    main()
